def build_system_prompt(system_prompt: str, *, shopify_tools_enabled: bool = False) -> str:
    ...
    if shopify_tools_enabled:
        support = (
            ...
            "- The numbered excerpts below are **supplementary**; they may not contain order, inventory, or live catalog answers.\n"
            "- When Shopify tools are enabled, **call them** for this store’s orders, tracking, stock, products, or customer history "
            "before deciding the answer is unknown.\n"
            "- Use earlier messages in this thread for follow-ups (e.g. “it”, “that jacket”, “the sale you mentioned”) "
            "or to stay consistent with what you already said.\n"
            "- Be concise: short paragraphs or bullets when listing options. Do not fabricate products, prices, policies, or contact details "
            "that are not supported by tool results, the excerpts, or the visible thread.\n"
            "- If excerpts include a concrete fact (for example a price like 'PKR 3,490'), you may use it when it applies.\n"
            "- Only use the fallback when tools (if applicable) and excerpts do not support a concrete answer."
        )
    else:
        support = (
            "You are a customer-support chatbot for this brand. Your job is to help shoppers and visitors "
