    ...
    *,
    ...
    if has_shopify_tools:
        ...
        shopify_line = (
            ...
            "  · `shopify_product_search` — catalog browse, recommendations, single-item lookup, "
            "\"do you sell/have…\", \"what products do you sell\", showing product cards. "
            "Not for order tracking, stock-only checks, catalog math (averages, cheapest across many items), "
            "purchase promotions/perks/freebies, or questions about what **you** (the assistant) can do.\n"
            "  · `shopify_catalog_query` — averages, cheapest/most expensive, price ranges, counts, "
            "or any question needing math across multiple products. Answer in text from `analysis` stats; "
            "do not expect product cards. Use `filter_query` like product search (`snowboard`, "
            "`published_status:published`, etc.) and `question` mirroring what they asked.\n"
        ...
        shopify_line += (
            ...
            "  · For multi-part catalog questions (e.g. snowboards plus average price), call "
            "`shopify_catalog_query` for stats and `shopify_product_search` only if they also asked to see products.\n"
            "  · For a single named product's price, answer in one short sentence from tool results. "
            "For cheapest, average, or range across the catalog, use `shopify_catalog_query` and cite its stats."
        )
