...
export function WidgetLauncher({
  ...
  const chrome = brandChromeClasses(brandColorHex);
  const hasBrand = Boolean(parseBrandColorHex(brandColorHex));
  const radius = WIDGET_BORDER_RADIUS_DEFAULT;
  ...
  return (
    <button
      ...
      className={cn(
        "demo-widget-launcher relative flex size-14 items-center justify-center border border-black/10 shadow-[0_0_0_4px_#fff,0_10px_25px_rgba(15,23,42,0.22)] transition-[transform,opacity,visibility] duration-200 ease-out hover:-translate-y-px",
        !visible && "pointer-events-none invisible scale-90 opacity-0",
        className,
      )}
      style={{
        backgroundColor: brandColorHex,
        borderRadius: `${radius}px`,
        ["--widget-preview-radius" as string]: `${radius}px`,
        ["--widget-preview-accent" as string]: brandColorHex,
      }}
    >
      <span
        className="widget-preview-launcher-surface flex items-center justify-center"
      ...
      <span className="widget-preview-launcher-attention" aria-hidden />
      <span className={cn("sr-only", chrome?.fabIconClass)} />
    </button>
  );
}
