...
export type WidgetWelcomeScreenProps = {
  ...
  /** Override welcome gradient (demo outreach uses static ChatRely blue). */
  panelGradient?: string | null;
  className?: string;
  onChatClick?: () => void;
};
...
export function WidgetWelcomeScreen({
  ...
  const background = panelGradient?.trim() || welcomePanelGradient(brand);
        ...
        <div className="px-4 pt-0 sm:px-4">
          <div className="rounded-2xl border border-slate-200/95 bg-white/[0.97] p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            ...
            <button
              type="button"
              className={cn(
                "mt-3.5 w-full rounded-xl px-4 py-2.5 text-[14px] font-semibold transition-opacity hover:opacity-90 active:opacity-80 touch-manipulation",
